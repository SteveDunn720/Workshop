...
class Constraint:
    ...
    def set_weight(self, driver: str, weight: float) -> None:
        ...
        if driver not in weights:
            raise ValueError(
                f"{driver!r} is not a driver of constraint {self.node!r}."
            )
...
class ParentConstraint(Constraint):
    ...
    def weights(self) -> dict[str, str]:
        ...
        return {
            driver: f"{self.node}.{alias}"
            for driver, alias in zip(drivers, aliases, strict=False)
        }
    ...
    def add_driver(self, driver: str, weight: float = 1.0) -> str:
        ...
        if driver in self.drivers:
            cmds.warning(
                f"{driver} is already a driver of {self.node}."
            )
    ...
    def remove_driver(self, driver: str) -> None:
        ...
        if driver not in self.drivers:
            raise ValueError(
                f"{driver!r} is not a driver of {self.node!r}."
            )
